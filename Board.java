...
public class Board {
	...
	public boolean isDraw() {
		// loops through each cell to check if any are empty, if so, returns false
		for (int row = 0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
		...
		// If no empty cell was found to return false, then we can return true
		return true;
	}
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		// check if player has 3-in-the-diagonal (bottom left to top right)
